...
from main_8ch import get_args_parser
...
def draw_ref_pts_aux(img: np.ndarray, points: np.ndarray, color=(255, 255, 255)) -> np.ndarray:
    assert len(points.shape) == 3 and points.shape[2] == 2, 'invalid points shape: {}'.format(points.shape)
    num_points = points.shape[1]

    for i in range(300, num_points):
        # Get the initial point
        start_point = tuple(points[0, i].astype(int))
        color = COLORS_10[i%len(COLORS_10)]
        
        # Draw only lines for intermediate points
        for j in range(1, points.shape[0] - 1):
            next_point = tuple(points[j, i].astype(int))
            cv2.line(img, start_point, next_point, color, 1)
            start_point = next_point  # Update the start point for the next line
        
        # Draw an arrow for the final displacement
        end_point = tuple(points[-1, i].astype(int))
        cv2.arrowedLine(img, start_point, end_point, color, 1, tipLength=0.3)
...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'test/npy', self.seq_num))

        img_list = [os.path.join(self.args.mot_path, 'test/npy', self.seq_num, _) for _ in img_list if
                    ('jpg' in _) or ('png' in _) or ('npy' in _)]
        ...
        self.predict_path = os.path.join(self.args.output_dir, 'preds')
        self.predict_txt = os.path.join(self.predict_path, self.seq_num+'.txt')
        os.makedirs(self.predict_path, exist_ok=True)
        if os.path.exists(self.predict_txt):
            os.remove(self.predict_txt)

    def load_img_from_file(self, f_path):
        label_path = f_path.replace('images', 'labels_with_ids').replace('.png', '.txt').replace('.jpg', '.txt').replace('.npy', '.txt')
        if 'npy' in f_path:
            cur_img = np.load(f_path)
        else:
    ...
    @staticmethod
    def write_results(txt_path, frame_id, bbox_xyxyxyxy, identities, labels):
        save_format = '{frame},{id},{x1},{y1},{x2},{y2},{x3},{y3},{x4},{y4},-1,{label},-1\n'
        with open(txt_path, 'a') as f:
            for xyxyxyxy, track_id, label in zip(bbox_xyxyxyxy, identities, labels):
                if track_id < 0 or track_id is None:
                    continue
                x1, y1, x2, y2, x3, y3, x4, y4 = xyxyxyxy
                line = save_format.format(frame=int(frame_id), id=int(track_id), x1=x1, y1=y1, x2=x2, y2=y2, x3=x3, y3=y3, x4=x4, y4=y4, label=label)
                f.write(line)

    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'MOT15/images/train')
        result_filename = self.predict_txt
    ...
    def visualize_img_with_bbox_clscolor(img_path, img, dt_instances: Instances, ref_pts=None, gt_boxes=None, all_ref_pts_aux=None):
        ...
        if all_ref_pts_aux is not None:
            img_show = draw_ref_pts_aux(img_show, all_ref_pts_aux)
        cv2.imwrite(img_path, img_show)
    ...
    def detect(self, prob_threshold=0.7, area_threshold=100, vis=False):
        ...
        for i in tqdm(range(0, self.img_len)):
            ...
            all_ref_pts_aux = tensor_to_numpy(res['debug']['ref_pts_all'][:,0,:,:]) #shape [7, n 2]
            all_ref_pts_aux = all_ref_pts_aux * np.array([1200, 900])
            ...
            if vis:
                ...
                gt_boxes = None
                self.visualize_img_with_bbox_clscolor(cur_vis_img_path, ori_img, dt_instances, ref_pts=all_ref_pts, gt_boxes=gt_boxes, all_ref_pts_aux = all_ref_pts_aux)

            tracker_outputs = self.tr_tracker.update(dt_instances)
            box_xyxyxyxy = dt_instances.boxes
            identities = dt_instances.obj_idxes
            labels = dt_instances.labels
            self.write_results(txt_path=self.predict_txt,
                               frame_id=(i + 1),
                               bbox_xyxyxyxy=box_xyxyxyxy,
...
if __name__ == '__main__':
    ...
    # seq_nums = os.listdir(os.path.join(args.mot_path, 'test/rgb'))
    seq_nums = os.listdir(os.path.join(args.mot_path, 'test/npy'))
    # spec_seq = ['data46-4']
    spec_seq = []
    ...
    for seq_num in seq_nums:
        if spec_seq and seq_num not in spec_seq:
            print("skip {}".format(seq_num))
            continue
